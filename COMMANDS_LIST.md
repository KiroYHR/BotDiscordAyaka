# 🌸 Sổ Tay Lệnh Của Kamisato Ayaka
...
- **`!join`** (hoặc `!j`): Gọi Ayaka tham gia vào kênh thoại (Voice Channel) mà cậu đang có mặt.
- **`!play <từ khóa/link youtube>`** (hoặc `!p`): Yêu cầu Ayaka phát nhạc. (Ví dụ: `!play nhạc lofi`)
- **`!skip`** (hoặc `!s`): Bỏ qua bài hát hiện tại và chuyển sang bài tiếp theo trong hàng đợi.
...
- **`!hsr <UID>`** (hoặc `!starrail`): Tra cứu thông tin vé thông hành Honkai: Star Rail.
- **`!zzz <UID>`** (hoặc `!zenless`): Tra cứu thông tin người kết nối Zenless Zone Zero.

## 📚 5. AI Đọc Tài Liệu & Tóm Tắt (RAG)
- **`!askpdf`** (hoặc `!pdf`): Đọc và trả lời câu hỏi dựa trên file PDF (chỉ cần đính kèm file PDF kèm câu lệnh).
